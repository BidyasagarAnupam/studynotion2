...
exports.signup = async (req, res) => {
    try {
        ...
        console.log("Printing the req body", otp);

        // validate data
        if (!firstName || !firstName || !lastName || !email || !password || !confirmPassword
            || !otp) {
        ...
        console.log(`Our OTP - ${otp} and generate otp - ${recentOtp[0].otp}`);
        ...
        console.log("Error in Rgistration of User", error);
...
exports.login = async (req, res) => {
    try {
        ...
        if (!email || !password) {
            return res.status(403).json({
                success: false,
                message: 'All fields are required, pease try again'
            });
        }
